...
- **`node scripts/conformance.mjs`** — proves the Rust and Node data-format implementations agree, over `tests/fixtures/format/` (valid/ must parse identically, invalid/ must be rejected by BOTH) plus all real entries. When you change the format: update both implementations, add a fixture, keep this green (`/format-sync` walks through it).
