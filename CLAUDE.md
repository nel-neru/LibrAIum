...
- **Commands**: `/verify`, `/add-entry <github-url>`, `/curate-review`, `/format-sync`, `/utilize` (one PDCA iteration of the utilization loop — state in `.claude/utilization-backlog.md`), `/refresh-metadata` (headless GitHub metadata refresh via `scripts/refresh-metadata.mjs`; dry-run first), `/confirm-notes` (interview the owner to convert AI-drafted Personal Notes into firsthand ones; progress tracked in `.claude/notes-review.md`). **Agents**: `entry-curator`, `conformance-auditor`, `libraium-reviewer`. **Skill**: `entry-authoring` (house style for entries — consult it before writing any `data/entries/**/*.md`).
