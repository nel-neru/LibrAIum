...
- If commands, scripts, or structure changed, update AGENTS.md in the same commit.
...
- Update `.Codex/improvement-backlog.md`: move the item to Done with the commit hash; add any new issues discovered during the work to Pending (with priority); include the backlog update in the same commit.
