...
> **実装との差異（Deviations from shipped v1.0）**
> 本書は当初の設計意図の記録です。実装確定後に意図的に変更した点は以下のとおりで、
> 正典的な最新の挙動は `CLAUDE.md` を参照してください。
> - **Git Layer**: 本書は libgit2 / git2-rs を想定していますが、実装は**システムの `git` CLI をラップ**します（push/pull がユーザーの credential helper / SSH agent をそのまま利用できるため）。`git2` は依存に含みません（`src-tauri/src/gitops.rs`）。
> - **Background Tasks**: 図中の「Background Tasks（X収集・メタデータ更新）」は**廃止**しました。ネットワークアクセスは明示的なユーザー操作時のみで、バックグラウンド/スケジュール実行は行いません。メタデータ更新と Reception 収集はオンデマンドのコマンドです。
