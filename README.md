...
- **MCP server for Claude Code** — eight tools: `search_repos`, `get_repo_details`, `get_related`, `find_by_reception`, `suggest_for_new_project`, `compare_repos`, `get_library_overview`, `add_repo`.
...
| `find_by_reception` | Query the Reception moat by keyword and/or signal (migration / caution / adopter); returns matching entries with the evidence bullets |
